...
export function linkDailyBrokerAlias() {
  ensureRunDir();
  const alias = path.join(RUN_DIR, "broker.sock");
  try {
    const st = fs.lstatSync(alias);
    if (st.isSymbolicLink()) {
      const cur = fs.readlinkSync(alias);
      if (cur === "daily.sock" || cur === DEFAULT_DAILY_SOCKET_PATH) return alias;
      fs.unlinkSync(alias);
    } else {
      fs.unlinkSync(alias);
    }
  ...
  fs.symlinkSync("daily.sock", alias);
  return alias;
}

export function isProtectedDailySocket(socketPath) {
  if (!socketPath) return false;
  const base = path.basename(socketPath);
  if (base !== "daily.sock" && base !== "broker.sock") return false;
  return path.dirname(socketPath) === RUN_DIR;
}
