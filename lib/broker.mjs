...
import { SOCKET_PATH, AUDIT_LOG_PATH, ensureRunDir, isProtectedDailySocket } from "./paths.mjs";
...
export class Broker {
  ...
  start({ reuse = false } = {}) {
    ensureRunDir();
    if (!reuse && !isProtectedDailySocket(this.socketPath)) {
      try {
  ...
  async _listenExclusive(reuse) {
    try {
      ...
      if (!reuse || err.code !== "EADDRINUSE") {
        if (!isProtectedDailySocket(this.socketPath)) {
          try {
            fs.unlinkSync(this.socketPath);
          } catch {
            // ignore
