...
import { withSocketLock } from "./socket-lock.mjs";
...
export class Broker {
  ...
  start() {
    ensureRunDir();
    return withSocketLock(this.socketPath, () => this._listenExclusive());
  }

  async _listenExclusive() {
    try {
      await this._listen();
      return;
    } catch (err) {
  ...
  _drop(sock) {
    ...
    if (this.active === sock) {
      ...
      for (const [id, p] of this.pending) {
        const error = "extension disconnected; request outcome is unknown";
        if (p.reject) p.reject(new Error(error));
        else this._send(p.sock, { id: p.clientId, error });
        p.done?.();
  ...
  close() {
    ...
    // net.Server.close unlinks a socket it successfully bound. A losing
    // contender must not unlink the winner's pathname.
  }
  ...
  _forwardFromMcp(sock, msg) {
    ...
    const run = () => {
      // A queued action from a departed client has not been sent and can be
      // discarded. An already-dispatched action keeps its queue slot until
      // the extension replies or the original request times out.
      if (sock.destroyed || !this.mcpClients.has(sock)) return Promise.resolve();
      if (!this.active || this.active.destroyed) {
