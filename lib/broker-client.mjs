...
export class BrokerClient {
  ...
  waitReady(timeoutMs = 20000) {
    ...
    return withTimeout(this._ready, timeoutMs, "extension native port did not connect").then((ready) => {
      if (!this.active) throw new Error("broker socket closed while waiting for extension");
      return ready;
    });
  }
  ...
  async connect({ timeoutMs = 1500 } = {}) {
    ...
    sock.on("close", () => {
      ...
      this.hubPid = null;
      this._resetReady();
      for (const [, p] of this.pending) p.reject(new Error("broker socket closed; request outcome is unknown"));
      this.pending.clear();
    });
    sock.on("error", () => {});
...
export async function ensureDailyBroker({
  ...
  // Only the daemon may recover a stale socket, under its startup lock.
